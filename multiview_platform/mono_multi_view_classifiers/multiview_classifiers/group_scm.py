...
classifier_class_name = 'GroupSCM'
...
class GroupSetCoveringMachineClassifier(BaseSetCoveringMachine):
    ...
    def __init__(self, features_to_index, prior_rules, update_method, groups,
                 tiebreaker, p=1.0,
                 model_type="conjunction", max_rules=10, random_state=None):
        super(GroupSetCoveringMachineClassifier, self).__init__(p=p,
    ...
    def fit(self, X, y, tiebreaker=None, iteration_callback=None, **fit_params):
        ...
        self.classes_, y, total_n_ex_by_class = np.unique(y,
                                                          return_inverse=True,
                                                          return_counts=True)
        if len(self.classes_) != 2:
            raise ValueError("y must contain two unique classes.")
        logging.debug(
            "The data contains %d examples. Negative class is %s (n: %d) and positive class is %s (n: %d)." %
            (len(y), self.classes_[0], total_n_ex_by_class[0], self.classes_[1],
        ...
        while len(remaining_negative_example_idx) > 0 and len(
                self.model_) < self.max_rules:
            ...
            opti_P_bar = self._get_best_utility_rules(X,
                                                      ...
                                                      next_rule_model_idx=
                                                      self.model_.rules[
                                                          -1].feature_idx if len(
                                                          self.model_.rules) != 0 else None,
                                                      # int or NoneType
                                                      **utility_function_additional_args)
            iteration_info["utility_max"] = opti_utility
            iteration_info["utility_argmax"] = opti_feat_idx
            logging.debug(
                "Greatest utility is %.5f" % iteration_info["utility_max"])
            logging.debug("There are %d rules with the same utility." % len(
                iteration_info["utility_argmax"]))
            ...
            logging.debug(
                "Tiebreaking. Found %d optimal rules" % len(opti_feat_idx))
            ...
            if len(opti_feat_idx) > 1:
                if tiebreaker is None:
                    ...
                    if len(self.model_.rules) == 0:
                        ...
                        list_model_idx = [el.feature_idx for el in
                                          self.model_.rules]
                        rules_to_untie = [self.features_to_index[idx] for idx in
                                          list_model_idx]
                        print(
                            f'Yeah we use the tiebreaking and the rules to untie are {rules_to_untie}')
                        length_groups_of_rules_to_untie = [
                            len(self.groups[rule]) for rule in rules_to_untie]
                        keep_idx = np.argmin(length_groups_of_rules_to_untie)
            else:
                keep_idx = 0
            stump = DecisionStump(feature_idx=opti_feat_idx[keep_idx],
                                  threshold=opti_threshold[keep_idx],
                                  kind="greater" if opti_kind[
                                                        keep_idx] == 0 else "less_equal")
            ...
            logging.debug("There are %d examples remaining (%d negatives)" % (
            len(remaining_example_idx),
    ...
    def _get_best_utility_rules(self, X, y, X_argsort_by_feature_T, example_idx,
                                groups, groups_rules, features_weights,
                                next_rule_model_idx):
        if next_rule_model_idx is None:
            ...
            return find_max_utility(self.p, X, y, X_argsort_by_feature_T,
                                    example_idx, features_weights)
        else:
            previous_rule_choosed_i = self.features_to_index[
                next_rule_model_idx]  # must give a feature Name
            # UPDATE PRI FOR ALL THE RULES HAVING AT LEAST ONE OF THE GROUPS OF THE PREVIOUS RULES
            groups_previous_rule_choosed_i = self.groups[
                previous_rule_choosed_i]  # list
            ...
            dict_intersection_groups_rules = {feat_name: len(
                [el for el in groups_previous_rule_choosed_i if
                 el in groups_list]) for feat_name, groups_list in
                                              self.groups.items()}
            if self.update_method == 'inner_group':
                for idx, feat_name_idx in self.features_to_index.items():
                    if idx != next_rule_model_idx:  # pas la peine de reponderer la meme regle quoi
                        features_weights[idx] *= np.exp(
                            dict_intersection_groups_rules[feat_name_idx])
                ...
                raise ValueError(
                    f"{self.update_method} must be a str and in ['inner_group', 'outer_group']")
            # UPDATE GR
...
class GroupSCM(BaseMultiviewClassifier):
    """
    ...
    def fit(self, X, y, train_indices=None, view_indices=None):
        ...
        self.clf.fit(X=X, y=y[train_indices])
        self.groups_rules = self.clf.groups_rules

    def predict(self, X, example_indices=None, view_indices=None):
        _, X = self.transform_data_to_monoview(X, example_indices,
                                                           view_indices)
