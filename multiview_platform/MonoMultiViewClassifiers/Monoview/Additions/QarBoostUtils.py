...
from ..MonoviewUtils import format_y
from ... import Metrics



class ColumnGenerationClassifierQar(BaseEstimator, ClassifierMixin, BaseBoost):
    def __init__(self, n_max_iterations=350, estimators_generator=None, random_state=42, self_complemented=True, twice_the_same=False, old_fashioned=False,
                 previous_vote_weighted=True, c_bound_choice = True, random_start = True, two_wieghts_problem=False, divided_ponderation=True, n_stumps_per_attribute=None, use_r=True, plotted_metric=Metrics.zero_one_loss):
    ...
    def fit(self, X, y):
        ...
        formatted_y = format_y(y)

        if self.estimators_generator is None:
            self.estimators_generator = StumpsClassifiersGenerator(n_stumps_per_attribute=self.n_stumps_per_attribute,
                                                                   self_complemented=self.self_complemented)
        # Initialization
        self.get_classification_matrix(X, formatted_y)
        m, n = self.classification_matrix.shape

        formatted_y = formatted_y.reshape((m,1))
        y_kernel_matrix = np.multiply(formatted_y, self.classification_matrix)
        ...
        for k in range(min(n-1,
            ...
            self.record_infos(formatted_y, weighted_score)
    ...
    def record_infos(self, y, r):
        self.previous_vote = np.matmul(
            self.classification_matrix[:, self.chosen_columns_],
            np.array(self.voters_weights).reshape((k + 1, 1))).reshape(
    ...
    def compute_voter_weight(self, weighted_score, k):
        if self.divided_ponderation:
            if self.use_r:
                self.q = (1 / (self.n_max_iterations - k)) * 0.5 * math.log(
                    (1 + weighted_score) / (1 - weighted_score))
            else:
                self.q = (1 / (self.n_max_iterations - k)) * math.log(
                    (1 - weighted_score) / weighted_score)
        else:
            if self.use_r:
